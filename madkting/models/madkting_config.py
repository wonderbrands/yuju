...
class MadktingConfig(models.Model):
    ...
    simple_stock_locations = fields.Boolean('Obtiene stock desde campos calculados', default=True,
    ...
    webhook_product_batchsize = fields.Integer('Numero de productos por mensaje', default=100)
    webhook_product_limit = fields.Integer('Numero de mensajes a procesar', default=1)
    ...
    order_items_default_name = fields.Boolean('Usar nombre del item como nombre de la linea', help='Si esta habilitada, se utiliza la nombre del item como nombre de la linea de venta, en lugar del nombre del producto en el marketplace')
    shipping_webhook_enabled = fields.Boolean(string="Enviar webhook de guia envio", default=False)
    ...
    order_detail_enabled = fields.Boolean('Habilitar Detalle de Venta', default=True)
    ...
    service_url = fields.Char(string="Url servicio", required=True, default="https://internal.yuju.io/odoo")
    ...
    validate_order_duplicated_confirm = fields.Boolean('Validar order duplicada para confirmar', default=True, help='Si se habilita se validara que la orden no este duplicada al momento de confirmar')    
    webhook_stock_move_status = fields.Char(string="Webhook Status", help='Estados de los movimientos de stock para enviar webhook', default="done,assigned,cancel")
